...
public class PageableResultSet implements PageResultSet {
	...
	public int getPageCount() {
		...
		// 计算总页数
		int pageCount = (rowsCount % pageSize == 0) ? (rowsCount / pageSize)
	...
	public int getPageSize() {
		// 设置的每页显示的记录条数大于剩余条数时
		if (pageSize > getPageRowsCount()) {
			pageSize = getPageRowsCount();
		}
		return pageSize;
	}
	...
	public void pageFirst() throws SQLException {
		int row = (nowPage - 1) * pageSize + 1;
		rs.absolute(row);

	}

	public void pageLast() throws SQLException {
		int row = (nowPage - 1) * pageSize + getPageRowsCount();
		rs.absolute(row);
